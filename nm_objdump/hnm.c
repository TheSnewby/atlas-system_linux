...
/**
 * get_symbol_type - Determine symbol type based on ELF symbol attributes
 * @sym: Symbol entry
 * 
...
/**
 * Parse Symbol Table - Parse And Print ELF Symbol Table
...
int parse_symbol_table(const char *file_path)
{
    ...
    int is_big_endian, num_symbols, i;
    Elf64_Ehdr header;
    ...
    for (i = 0; i < header.e_shnum; i++)
	{
        ...
        /* Find Corresponding String Table */
            ...
            /* Ensure It's Not The Section Header String Table */
    ...
    if (symtab_shdr && strtab_shdr)
	{
        ...
        symbols = malloc(symtab_shdr->sh_size);
        if (!symbols)
		{
        ...
        if (fread(symbols, sizeof(Elf64_Sym), num_symbols, fptr) 
            != (size_t)num_symbols)
		{
        ...
        for (i = 0; i < num_symbols; i++)
		{
            ...
            symbols[i].st_value = swap_uint64(symbols[i].st_value, is_big_endian);
            symbols[i].st_shndx = swap_uint16(symbols[i].st_shndx, is_big_endian);

            /* Get Symbol Name */
            const char *name = &strtab[symbols[i].st_name];

            /* Get Symbol Type */
            char symbol_type = get_symbol_type(&symbols[i]);

            /* Print Symbol */
            if (symbols[i].st_value || symbol_type == 'U')
			{
                if (symbol_type == 'U')
                    printf("                 %c %s\n", symbol_type, name);
                else
                    printf("%016lx %c %s\n", symbols[i].st_value, symbol_type, name);
            }
        }
        free(symbols);
        free(strtab);
